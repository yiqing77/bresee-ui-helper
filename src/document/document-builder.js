    ...
    if (/\.md$/.test(doc)) {
      ...
      readLine.on('line', (line) => {
        ...
        if (line.includes('Scoped Slot')) {
          filename = 'bre-' + line.replace(/#/g, '').replace('Scoped Slot', '').trim().replace(/ /g, '-').toLocaleLowerCase()
          if (filename.length === 3) {
            filename = 'bre-' + doc.substr(0, doc.length - 3)
          }
          flag = 'scopedSlots'
        ...
        if (line.includes('Slot')) {
          filename = 'bre-' + line.replace(/#/g, '').replace('Slots', '').replace('Slot', '').trim().replace(/ /g, '-').toLocaleLowerCase()
          if (filename.length === 3) {
            filename = 'bre-' + doc.substr(0, doc.length - 3)
          }
          flag = 'slots'
